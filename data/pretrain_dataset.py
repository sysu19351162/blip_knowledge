...
class pretrain_dataset(Dataset):
    def __init__(self, ann_file, laion_path, cc3m_path, transform):
    ...
    def __getitem__(self, index):    
        ...
        if ann['image'][0] =='/':
            ...
            image = Image.open(os.path.join(self.cc3m_path,ann['image'])).convert('RGB')
...
class pretrain_knowledge_dataset(Dataset):
    def __init__(self, ann_file, image_knowledge_file, laion_path, cc3m_path, image_root, transform,knowledge_num,max_words):
        self.cc3m_path = cc3m_path
        self.ann_pretrain = []
        for f in ann_file:
            print('loading ' + f)
    ...
    def __getitem__(self, index):
        ...
        if ann['image_id'][0] == '/':
            image = Image.open(ann['image_id']).convert('RGB')
        else:
            image = Image.open(os.path.join(self.cc3m_path, ann['image_id'])).convert('RGB')
        image = self.transform(image)
        caption = pre_caption(ann['caption'], 30)
        knowledge = ''
        for en in ann['entity']:
            knowledge = knowledge + ',' + en
        caption = caption + ';related information:' + knowledge
        ...
        # if len(self.img_knowledge_pretrain[ann['image_id']]['knowledge_vg'])< self.knowledge_num:
        #     knowledge_num = len(self.img_knowledge_pretrain[ann['image_id']]['knowledge_vg'])
        # else:
        #     knowledge_num = self.knowledge_num
        # for i in  random.sample(self.img_knowledge_pretrain[ann['image_id']]['knowledge_vg'], knowledge_num):
