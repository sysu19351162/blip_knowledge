...
from data.chat_dataset import chat_coco_karpathy_train, chat_coco_karpathy_retrieval_eval
from data.nocaps_dataset import nocaps_eval
from data.flickr30k_dataset import flickr30k_train, flickr30k_retrieval_eval
from data.vqa_dataset import vqa_dataset
from data.nlvr_dataset import nlvr_dataset
from data.pretrain_dataset import pretrain_dataset, pretrain_knowledge_dataset
from data.visdial_dataset import visdial_dataset
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        dataset = pretrain_dataset(config['train_file'], config['laion_path'], config['cc3m_path'], transform_train)
        return dataset

    if dataset=='pretrain_knowledge':
        dataset = pretrain_knowledge_dataset(config['train_file'], config['image_knowledge'], config['laion_path'],
                                             config['cc3m_path'], config['image_root'], transform_train, config['knowledge_num'],
                                             config['max_words'])
        return dataset
    ...
    elif dataset == 'chat_kg_coco':
        train_dataset = chat_coco_karpathy_train(transform_train, config['image_root'], config['ann_root'])
        val_dataset = chat_coco_karpathy_retrieval_eval(transform_test, config['image_root'], config['ann_root'], 'val')
        test_dataset = chat_coco_karpathy_retrieval_eval(transform_test, config['image_root'], config['ann_root'], 'test')
        return train_dataset, val_dataset, test_dataset
    ...
    elif dataset=='visdial':
        train_dataset = visdial_dataset(transform_train, '/data1/yangzhenbang_new/datasets/visdial','train')
        val_dataset = visdial_dataset(transform_test, '/data1/yangzhenbang_new/datasets/visdial','val')
        test_dataset = visdial_dataset(transform_test, '/data1/yangzhenbang_new/datasets/visdial','test')
        return train_dataset, val_dataset, test_dataset
