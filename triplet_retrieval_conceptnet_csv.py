...
df = pd.read_csv('to_df.csv',chunksize=10000,header=None)
...
for ch in df:
    vcs = ch['vc']#取出每一列的visual_concept
    for i in tqdm(range(len(vcs)):
        # for i in tqdm(range(2)):
        entity_set = vcs[i].split(',')
                    ...
                    for other_entity_item in entity_set:
                        if other_entity_item != entity_item:
                            for knowledge_item in knowledge_value:
                                ...
                                if triplet[0] == entity_item and triplet[-1] == other_entity_item or triplet[
                                    0] == other_entity_item and triplet[-1] == entity_item:
